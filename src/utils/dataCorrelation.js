...
export class DataCorrelationService {
     ...
     */
    correlateSearchResults(hubspotCompanies, hubspotContacts, dwollaCustomers, dwollaTransfers = []) {
        const correlatedData = [];
        const processedDwollaIds = new Set();
        // First pass: Match by dwolla_id (highest confidence)
        hubspotCompanies.forEach(company => {
            if (company.properties.dwolla_id) {
                const dwollaCustomer = dwollaCustomers.find(customer => customer.id === company.properties.dwolla_id);
                if (dwollaCustomer) {
                    ...
                    correlatedData.push({
                        ...
                        dwolla: {
                            ...
                            transfers: dwollaTransfers.filter(t => t.source.id === dwollaCustomer.id ||
        ...
        hubspotContacts.forEach(contact => {
            if (contact.properties.email) {
                const dwollaCustomer = dwollaCustomers.find(customer => customer.email.toLowerCase() === contact.properties.email.toLowerCase() &&
        ...
        hubspotCompanies.forEach(company => {
            if (!company.properties.dwolla_id) {
                dwollaCustomers
                    ...
                    .forEach(customer => {
                    const similarity = this.calculateNameSimilarity(company.properties.name, customer.businessName || '');
                    if (similarity > 0.8) {
                        ...
                        correlatedData.push({
                            ...
                            dwolla: {
                                customer,
                                transfers: dwollaTransfers.filter(t => t.source.id === customer.id ||
        ...
        hubspotCompanies.forEach(company => {
            const isCorrelated = correlatedData.some(data => data.hubspot.company?.id === company.id);
            if (!isCorrelated) {
                const companyContacts = this.findRelatedContacts(company, hubspotContacts);
                correlatedData.push({
                    hubspot: {
        ...
        hubspotContacts.forEach(contact => {
            const isCorrelated = correlatedData.some(data => data.hubspot.contacts.some(c => c.id === contact.id));
            if (!isCorrelated) {
                correlatedData.push({
                    hubspot: {
                        company: undefined,
     ...
     */
    findInconsistencies(company, customer) {
        const inconsistencies = [];
        // Check business name match
        if (customer.businessName && company.properties.name) {
            const nameSimilarity = this.calculateNameSimilarity(company.properties.name, customer.businessName);
     ...
     */
    findContactInconsistencies(contact, customer, company) {
        const inconsistencies = [];
     ...
     */
    findRelatedContacts(company, contacts) {
        return contacts.filter(contact => contact.properties.company?.toLowerCase() === company.properties.name?.toLowerCase());
    }
    /**
     * Find company related to a contact
     */
    findRelatedCompany(contact, companies) {
        if (!contact.properties.company)
            return undefined;
        return companies.find(company => company.properties.name?.toLowerCase() === contact.properties.company?.toLowerCase());
    }
}
