import { CorrelatedCustomerData } from './dataCorrelation'
...
export interface ActionTemplate {
  ...
  description: string
  icon: string
  category: 'copy' | 'link' | 'support' | 'verification' | 'transfer'
  conditions: (data: CorrelatedCustomerData) => boolean
  execute: (data: CorrelatedCustomerData) => Promise<ActionResult>
...
export class QuickActionsService {
   ...
   */
  getApplicableActions(data: CorrelatedCustomerData): ActionTemplate[] {
   ...
   */
  async executeAction(actionId: string, data: CorrelatedCustomerData): Promise<ActionResult> {
    ...
    try {
      ...
      logger.error('Quick action error', error as Error, { actionId })
      return {
        success: false,
   ...
   */
  registerAction(template: ActionTemplate): void {
  ...
  private initializeDefaultTemplates(): void {
    ...
    this.registerAction({
      ...
      execute: async (data) => {
        ...
        const summary = transfers.map(t => 
          `${t.status.toUpperCase()}: $${t.amount.value} (${new Date(t.created).toLocaleDateString()})`
        ).join('\n')
   ...
   */
  private generateCustomerSummary(data: CorrelatedCustomerData): string {
