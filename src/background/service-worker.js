...
async function handleSearch(query) {
    ...
    try {
        ...
        const correlatedData = dataCorrelationService.correlateSearchResults(hubspotData.companies, hubspotData.contacts, dwollaData.customers, dwollaData.transfers);
...
async function searchHubSpot(query, _token) {
...
async function searchDwolla(query, _token) {
...
async function handleGetTransfers(customerId) {
    try {
        // Import auth utility
        const { getAccessToken } = await import('../utils/auth');
...
async function getDwollaTransfers(customerId, _token) {
    const { EnhancedDwollaClient } = await import('../utils/apiEnhanced');
    const client = new EnhancedDwollaClient();
    try {
...
// Debug handlers
async function handleGetLogs(filter) {
