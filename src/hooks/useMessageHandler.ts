...
export function useMessageHandler() {
  ...
  const sendMessage = useCallback(async <T = any>(
    ...
    const attemptRequest = async (retriesLeft: number): Promise<T> => {
      try {
        // Create timeout
        ...
        if (retriesLeft > 0 && !controller.signal.aborted) {
          ...
          await new Promise(resolve => 
            setTimeout(resolve, Math.pow(2, retries - retriesLeft) * 1000)
          )
        ...
        // Log final error
        logger.error('Message request failed', error as Error, {
          message: message.type,
          timeout,
          retries
        })
